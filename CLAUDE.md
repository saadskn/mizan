...
**Mizan · ميزان** — a fully client-side "reverse macro calculator": the user enters remaining daily macros (protein/carbs/fats/calories) and the app brute-forces real 1–3 item combos from Riyadh fast-food chains, ranked by closeness to those targets. No backend, no state library; navigation is a hand-rolled hash router (`src/lib/route.js` parser + `src/hooks/useHashRoute.js` hook) with five views behind the ☰ menu: `#/` finder (home), `#/build` live meal builder (pick a chain, stepper per item, sticky live macro totals, persisted in `mm_build`), `#/needs` daily-needs calculator (`src/lib/needs.js`, Mifflin-St Jeor × activity = maintenance, shifted by a goal pill — fat loss −300–500 kcal / maintain / weight gain +300–500 kcal — so calories are also a range; then protein 1.6–2.2 g/kg, fat 20–25% men / 25–30% women, carbs remainder, all as ranges), `#/spots` restaurant directory, `#/spots/<slug>` per-chain menu tables. Chain slugs come from `src/lib/slug.js` (uniqueness locked by test); logos are static `public/logos/<slug>.png` files (~35 sourced; `SpotLogo.jsx` falls back to a monogram tile via img onError for the rest — a missing logo is never an error). The disclaimer footing renders on every page except `#/needs` (`Disclaimer.jsx`; the Daily Needs page shows no menu prices/macros, so the user had it removed there). Repo, package, and URL are all `mizan` (renamed from the working name `macromenu-riyadh` on 2026-07-11 — old Pages links are dead, git redirects still work). Roster and item counts are locked by tests in `src/data/index.test.js` — treat those assertions as the source of truth, and update them deliberately with any roster change. New chains are vetted against their live HungerStation Riyadh menu: only added if per-item calories are listed there and pass a sanity check (PizzaExpress, Alkhafeef, and Burger Hunch joined this way on 2026-07-14; Burger Hunch's "double" burgers were dropped for implausible listed calories).
